...
int64_t Storage::get_current_time_ms() const {
    return std::chrono::floor<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
}
...
void Storage::set(const std::string key, const std::string value) {
    ...
    if (config::DEFAULT_VALUE_TTL != -1)
        expires_at = get_current_time_ms() + static_cast<int64_t>(config::DEFAULT_VALUE_TTL) * 1000;
    storage_[std::move(key)] = {std::move(value), expires_at};
}
...
void Storage::active_clean(int32_t max_clean_per_loop) {
    ...
    if (!storage_.empty()) {
        ...
        for (auto cleanup_it = storage_.begin(); cleanup_it != storage_.end() && checked < max_clean_per_loop;) {
